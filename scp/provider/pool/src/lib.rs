...
pub use exposure::{ExposureEstimate, ExposureDistribution};
...
pub use metrics::{epoch_similarity, exposure_divergence, ConvergencePressure, EpochPhase};
    ...
    pub fn sample(&self, rng: &mut impl RngCore) -> ProviderQuorum<P> {
        ...
        match &self.strategy {
            SamplingStrategy::RandomK(k) => {
                if n == 0 { return ProviderQuorum::new(); }
                let k = (*k).min(n);
                ...
                let selected: Vec<([u8; 32], P)> = live[..k]
                    .iter()
            ...
            SamplingStrategy::WeightedComposite { k, influence, floor, liveness_discount } => {
                ...
                for _ in 0..k {
                    ...
                    let (provider_idx, _) = available.swap_remove(sel);
                    selected.push((self.active[provider_idx].0, self.active[provider_idx].1.clone()));
                }
                let ids: Vec<[u8; 32]> = selected.iter().map(|(id, _)| *id).collect();
                self.exposure_tracker.lock().unwrap().record(&ids);
                ProviderQuorum::from_providers(selected)
